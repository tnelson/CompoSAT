...
public class AmalgamUI {

    // TODO clean up all these static fields used to quickly get around alloyGUI code hangups
    static Module root;
    static A4Solution ans;
    static String lastPrefix;
    static TupleInExpr test;
    static A4Solution flipped;
    static Map<Expr, ProvenanceTree> treesForTopLevel = new HashMap<Expr, ProvenanceTree>();
    static List<ProvenanceTree> provenanceTrees = new ArrayList<ProvenanceTree>();
    static List<ProvenanceTree> literalProvenanceTrees = new ArrayList<ProvenanceTree>();
    ...
    public static String whyLN(SwingLogPanel log, Module newroot, A4Solution newans, String prefix, TupleInExpr testtup) throws Err {
        root = newroot;
        ans = newans;
        lastPrefix = prefix;
        test = testtup;
        ...
        for(Expr f : fmlas) {
            // XXX flipeval was encountering null pointer exceptions
            Object flipeval = null;
            //try {
                flipeval = flipped.eval(f);
                //System.out.println("evaluating "+f+"; got "+flipeval);
            //}
            //catch (Exception e) { System.err.println("Fatal exception encountered: "+e.toString());
              //                    e.printStackTrace(); }

            if(Boolean.FALSE.equals(flipeval)) {
                // XXX: Note new visitor for each top level; will miss differing exprs
                result.append("----------------------------------------\n");
    ...
    public static String finalizeProvenances() throws Err {
        ...
        if(A4Preferences.LiteralProv.get() > 0) {
            for(ProvenanceTree t : provenanceTrees) {
                //  FIXME doesn't handle case where split branches being taken
    ...
    public static void doLNTesting(SwingLogPanel log, Module root, A4Solution ans, TupleInExpr test, StringBuilder result, boolean sayLUN, String prefix) throws Err {
        try {
    ...
    static Set<TupleInExpr> getTestableTuples(Module root, A4Solution ans, boolean b) {
        Set<TupleInExpr> toTest = new HashSet<TupleInExpr>();
        ...
        if(b) {
            for(Sig s: root.getAllReachableSigs()) {
                ...
                if(!s.equals(Sig.UNIV) && !s.equals(Sig.NONE) && !s.equals(Sig.STRING) &&
                    ...
                    // XXX Clumsy forbidding of Ord/first, Ord/next etc.
                    ...
                    if(!s.hasChildren()) {
                        ...
                        for(A4Tuple t : ans.eval(s)) {
                            toTest.add(new TupleInExpr(s, t, false, b));
                        }
                    }
                    ...
                    for(Field f : s.getFields()) {   // (getFields gives the field exprs, not getFieldDecls)
                        ...
                        for(A4Tuple t : ans.eval(f)) {
                            ...
                            toTest.add(new TupleInExpr(f, t, !(sigF instanceof Relation) && s.isOne != null, b));
                        }
                ...
                if(!s.equals(Sig.UNIV) && !s.equals(Sig.NONE) && !s.equals(Sig.STRING) &&
                    ...
                    if(!s.hasChildren()) {
                        ...
                        for(Tuple kkt : kodkodUpperSig) {
                            ...
                            if(!found) toTest.add(new TupleInExpr(s, new A4Tuple(kkt, ans), false, b));
                        }
                    }
                    ...
                    for(Field f : s.getFields()) {
                        ...
                        for(Tuple kkt : kodkodUpperField) {
                            ...
                            if(!found) toTest.add(new TupleInExpr(f, new A4Tuple(kkt, ans), truncated, b));
                        }
    ...
    static TupleInExpr getATuple(Module root, A4Solution ans, boolean positiveTuples) throws Err {
        A4Tuple tuple = null;
        Expr relation = null;
        ...
        Set<TupleInExpr> testable = getTestableTuples(root, ans, positiveTuples);
        for(TupleInExpr te : testable) {
        ...
        return new TupleInExpr(relation, tuple, isOneTruncated, positiveTuples);
    }
