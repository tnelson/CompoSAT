...
public class A4Preferences {
   ...
   public static class ChoicePref<T> extends Pref<T> {
      ...
      protected Object[] checkChoices(Iterable<T> validChoices, T[] defValueCandidates) {
         ...
         if (defaultValue == null) { 
             if(lst.size() < 1) {                 
                 OurDialog.fatal("Amalgam could not load any supported SAT solvers! This is an unusual situation; please contact tn@cs.brown.edu. Exiting...");
                 System.exit(1);
             }
   ...
   /** AMALGAM: provenance display style: 1 for simple, 2 for expert */
   public static final IntChoicePref ProvStyle = new IntChoicePref("ProvStyle", "Provenance Detail", Arrays.asList(1, 2, 3), 1);

   /** AMALGAM: symmetry breaking */
   public static final IntChoicePref SBLevel = new IntChoicePref("SBLevel", "Symmetry Breaking Size", Arrays.asList(0, 20, 200, 2000, 20000), 20);

   /** AMALGAM: compute literal provenance */
   public static final IntChoicePref LiteralProv = new IntChoicePref("LiteralProv", "Compute Literal Provenance?", Arrays.asList(0, 1), 0);
